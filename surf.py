...
def main():
    parser = argparse.ArgumentParser(description = "File system interface for DigitalOcean droplets",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--config', help = 'Configuration file name',
                        default = '/etc/surf.conf.yaml')
    parser.add_argument('--mpoint', help = 'Mount point of the file system', required = True)
    parser.add_argument('--storage', help = 'Backing storage for easy passthrough', required = True)
